...
public class Light2D : MonoBehaviour
{
    ...
    private static void CulcShadowPoint(List<ShadowPoint> shadowPoints, Vector2 direction, ColliderMoreData foundCollider, Vector2 position)
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            Point point = ShadowRayCast(position, dir, foundCollider.AllPointsInWorldSpace, foundCollider.Light2DCollider.SelfShadows);
            if (point.IsHit)
            {
    ...
    private static Point ShadowRayCast(Vector2 origin, Vector2 direction, Vector2[] lightColliderAllPointsWorldSpace, bool selfShadows)
    {
        List<Vector2> points = new();
        for (int i = 0; i < lightColliderAllPointsWorldSpace.Length; i++)
        {
            float x1 = lightColliderAllPointsWorldSpace[i].x;
            float y1 = lightColliderAllPointsWorldSpace[i].y;
            float x2;
            float y2;
            if (i != lightColliderAllPointsWorldSpace.Length - 1)
            {
                x2 = lightColliderAllPointsWorldSpace[i + 1].x;
                y2 = lightColliderAllPointsWorldSpace[i + 1].y;
            }
            else
            {
                x2 = lightColliderAllPointsWorldSpace[0].x;
                y2 = lightColliderAllPointsWorldSpace[0].y;
            }
