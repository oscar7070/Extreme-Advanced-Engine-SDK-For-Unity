...
////// Emissive:
                ...
                float3 node_9319 = (_Cookie_var.a*_Cookie_var.rgb);
                float4 node_2957_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_2957_p = lerp(float4(float4(node_9319,0.0).zy, node_2957_k.wz), float4(float4(node_9319,0.0).yz, node_2957_k.xy), step(float4(node_9319,0.0).z, float4(node_9319,0.0).y));
                float4 node_2957_q = lerp(float4(node_2957_p.xyw, float4(node_9319,0.0).x), float4(float4(node_9319,0.0).x, node_2957_p.yzx), step(node_2957_p.x, float4(node_9319,0.0).x));
